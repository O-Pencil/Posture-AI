...
```bash
npm run verify              # Agent/提交前权威检查，CI 同步执行
npm run tsc:rn              # TypeScript 类型检查
npm test -- --runInBand     # Jest 单测（10 套件 79 测试）
npm run lint                # ESLint
npm run check:design        # 设计纪律（primitives 必须用 / 散落样式禁用）
npm run check:tokens        # token 化率 ≥ 40%
npm run check:deps          # 循环依赖
npm run check:exports       # 死导出
npm run check:colors        # 色值字面量（warn-only）
```

`npm run verify` 是 Agent 收尾与 GitHub Actions 的共同门禁；除非任务只改纯文档，否则默认跑它。
